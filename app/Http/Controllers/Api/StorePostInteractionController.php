...
class StorePostInteractionController extends Controller
{
    ...
    {
        $comments = StorePostComment::with('user:id,username,first_name,last_name,avatar')
            ->where('store_post_id', $post->id)->latest()->get()->map(fn ($comment) => [
                'id' => $comment->id,
                'body' => $comment->body,
                'author' => trim(($comment->user?->first_name ?? '').' '.($comment->user?->last_name ?? '')) ?: ($comment->user?->username ?? 'User'),
