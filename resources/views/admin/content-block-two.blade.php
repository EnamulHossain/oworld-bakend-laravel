...
@section('content')
    ...
    <form id="block-two-form" method="post" action="{{ route('admin.settings.content.block-two.update') }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row mb-3">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark">Content Block 2</h1>
                <small class="text-muted">Manage the 6-card grid for homepage.</small>
            </div>
            <div class="col-sm-6 text-right">
                <div class="content-actions">
                    <button type="button" class="btn btn-outline-secondary" id="add-card-btn">Add Card</button>
        ...
        <div class="content-surface">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <p class="badge-soft mb-1">Block 2</p>
                    <h4 class="mb-0">Cards</h4>
                    <small class="text-muted">Max 6 cards recommended.</small>
                </div>
            </div>
            <div id="cards-container" class="row"></div>
        </div>

        <input type="hidden" name="block_two" id="block_two_field">
    </form>

    <script>
        (function(){
            const initial = @json($blockTwo ?: []);
            const cards = Array.isArray(initial) ? initial : [];
            const categories = @json($categories);
            const events = @json($events);
            const offers = @json($offers);
            const container = document.getElementById('cards-container');
            const hidden = document.getElementById('block_two_field');
            const addBtn = document.getElementById('add-card-btn');
            const blank = () => ({ type:'category', category:'', title:'', subtitle:'', image:'', link:'' });

            const optionsByType = { category: categories, event: events, offer: offers };
            ...
            function render() {
                ...
                cards.forEach((card, idx) => {
                    ...
                    col.innerHTML = `
                        <div class="content-card">
                            ...
                            <div class="form-group mb-2">
                                <label>Title</label>
                                <input class="form-control" data-field="title" data-idx="${idx}" value="${card.title || ''}" placeholder="Card title">
                            </div>
                            <div class="form-group mb-2">
                            ...
                            <div class="form-group mb-2">
                                ...
                                <div class="custom-file">
                                    <input type="file" class="custom-file-input upload-input" data-preview="block2_preview_${idx}" name="block2_image_${idx}" id="block2_image_${idx}" accept="image/*">
                                    <label class="custom-file-label" for="block2_image_${idx}">Choose image</label>
                                </div>
                                <small class="form-text text-muted">Uploaded image overrides URL below.</small>
                                <div class="mt-2" id="block2_preview_${idx}">
                ...
                if (cards.length === 0) {
                    ...
                    col.textContent = 'No cards yet. Add one to begin.';
                    container.appendChild(col);
                }
    ...
    <script>
        document.addEventListener('change', (e) => {
            ...
            if (previewId && file) {
                ...
                const idx = previewId.replace('block2_preview_', '');
