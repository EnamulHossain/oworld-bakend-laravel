...
@section('content')
    <section class="bg-gradient-to-br from-indigo-50 via-white to-rose-50">
        <div class="mx-auto max-w-5xl px-4 py-16 lg:px-6">
            ...
            <p class="mt-3 text-lg text-slate-600">Discover new places, experiences, and offers — categorized for every mood
                and occasion.</p>
        </div>
    </section>
    ...
    <div class="mx-auto max-w-7xl px-4 py-12 lg:px-6">
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($categories as $category)
                <a class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
                    ...
                    <div class="space-y-2 p-4">
                        <div
                            class="inline-flex items-center gap-2 rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">
                            {{ $category->icon ?? '🗂️' }} {{ $category->short_name ?? $category->name }}</div>
