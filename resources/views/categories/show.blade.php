...
@section('content')
    <section class="bg-gradient-to-br from-indigo-50 via-white to-rose-50">
        <div class="mx-auto grid max-w-7xl grid-cols-1 items-center gap-8 px-4 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:px-6">
            <div class="space-y-4">
                <span
                    class="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1 text-sm font-semibold text-indigo-600 shadow-sm ring-1 ring-indigo-100">{{ $category->icon ?? '🗂️' }}
                    Category</span>
                <h1 class="text-3xl font-black text-slate-900 sm:text-4xl">{{ $category->name }}</h1>
                <p class="text-lg text-slate-600">
                    {{ $category->description ?? 'Experience the best picks from this curated section.' }}</p>
            ...
            @forelse ($events as $event)
                <div
                    ...
                    @if ($event->display_banner)
                        <img class="h-40 w-full object-cover" src="{{ $event->display_banner }}"
                            alt="{{ $event->name }}">
                    @endif
                </div>
            @empty
                <div class="rounded-xl border border-dashed border-slate-200 bg-white p-6 text-center text-slate-600">No
                    events have been published for this category yet.</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="mx-auto max-w-7xl px-4 py-12 lg:px-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div
                    ...
                    <div class="space-y-2 px-4 py-3">
                        ...
                        <div class="text-sm text-slate-500 space-y-1">
                            ...
                            <div>Valid from {{ optional($offer->start_date)->format('M d') }} to
                                {{ optional($offer->end_date)->format('M d, Y') }}</div>
                        </div>
                    </div>
            ...
            @empty
                <div class="rounded-xl border border-dashed border-slate-200 bg-white p-6 text-center text-slate-600">No
                    offers listed for this category right now.</div>
            @endforelse
        </div>
    </div>
@endsection
