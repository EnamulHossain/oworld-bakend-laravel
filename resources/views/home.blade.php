...
@section('content')
    ...
    <section class="mx-auto max-w-6xl px-4 py-12 lg:px-6">
        <div class="flex items-center justify-between gap-4">
            ...
            <a class="text-sm font-semibold text-indigo-600 hover:text-indigo-700" href="{{ route('offers.index') }}">Browse all</a>
        </div>
        <div class="mt-6">
            <div class="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 scrollbar-hide" id="offers-reels" data-auto-track>
                @forelse($offers as $offer)
                    @php
                        ...
                        $thumb = $offer->thumbnail ?? $offer->cover ?? $offer->display_image ?? ($gallery[0] ?? null);
                    @endphp
                    <article class="group relative w-52 flex-none snap-start overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg cursor-pointer" data-reel
                        ...
                        data-tag="{{ $offer->offer_type ?? 'Offer' }}">
                        <div class="relative h-64 overflow-hidden bg-slate-900">
                            @if(!empty($thumb))
                                <img class="h-full w-full object-cover transition duration-300 group-hover:scale-105" src="{{ $thumb }}" alt="{{ $offer->name }}">
                            @else
                                <div class="flex h-full items-center justify-center bg-slate-800 text-4xl text-white">🎁</div>
                            @endif
                            <span class="absolute left-2 top-2 rounded-full bg-white/90 px-2 py-1 text-[11px] font-semibold text-slate-900 shadow">{{ $offer->offer_type ?? 'Offer' }}</span>
                        </div>
                    </article>
                @empty
                    <div class="w-full rounded-xl border border-dashed border-slate-200 bg-white p-6 text-center text-slate-600">No offers yet.</div>
                @endforelse
            </div>
        </div>
    </section>
    ...
    <section class="mx-auto max-w-6xl px-4 py-12 lg:px-6">
        <div class="flex items-center justify-between gap-4">
            ...
            <a class="text-sm font-semibold text-indigo-600 hover:text-indigo-700" href="{{ route('categories.index') }}">View all</a>
        </div>
        <div class="mt-6 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 scrollbar-hide" data-auto-track>
            @foreach ($categories as $category)
                <a class="group relative w-52 flex-none snap-start overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg" href="{{ route('categories.show', $category) }}">
                    @if($category->display_image)
                        <img class="h-52 w-full object-cover transition duration-300 group-hover:scale-105" src="{{ $category->display_image }}" alt="{{ $category->name }}">
                    @else
    ...
    <section class="mx-auto max-w-6xl px-4 py-12 lg:px-6">
        <div class="flex items-center justify-between gap-4">
            ...
            <a class="text-sm font-semibold text-indigo-600 hover:text-indigo-700" href="{{ route('events.index') }}">All events</a>
        </div>
        <div class="mt-6 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 scrollbar-hide" data-auto-track>
            @forelse ($events as $event)
                <article class="flex h-full w-52 flex-none snap-start flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                    @if($event->display_banner)
                        <img class="h-52 w-full object-cover" src="{{ $event->display_banner }}" alt="{{ $event->name }}">
                    @else
            ...
            @empty
                <div class="w-full rounded-xl border border-dashed border-slate-200 bg-white p-6 text-center text-slate-600">No events published yet.</div>
            @endforelse
        </div>
    </section>
...
@if(!empty($blockOne))
    <section class="mx-auto max-w-6xl px-4 py-12 lg:px-6">
        ...
        <div class="mt-4 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 scrollbar-hide" data-auto-track>
            @foreach($blockOne as $item)
                <article class="flex h-full w-52 flex-none snap-start flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                    @if(!empty($item['image']))
                        <img class="h-52 w-full object-cover" src="{{ $item['image'] }}" alt="{{ $item['title'] ?? 'Card' }}">
                    @else
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach(collect($blockTwo)->take(6) as $item)
                <article class="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                    @if(!empty($item['image']))
                        <img class="h-48 w-full object-cover" src="{{ $item['image'] }}" alt="{{ $item['title'] ?? 'Card' }}">
                    @else
    ...
    <div id="reels-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 px-4 py-10">
        <div class="relative w-full max-w-3xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            <button type="button" id="reels-close" class="absolute right-3 top-3 inline-flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-lg font-bold text-slate-600 shadow hover:bg-slate-200" aria-label="Close reel">×</button>
            <div class="p-6">
                ...
                <div class="relative mt-4 flex items-center justify-center">
                    <button id="reels-prev" type="button" class="absolute left-2 inline-flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-slate-700 shadow hover:bg-slate-200">‹</button>
                    <div class="overflow-hidden rounded-2xl bg-slate-900 p-3">
                        <img id="reels-player-image" class="mx-auto h-[520px] w-[320px] rounded-xl object-cover" alt="Offer reel">
                        <video id="reels-player-video" class="mx-auto h-[520px] w-[320px] rounded-xl object-cover hidden" playsinline muted controls></video>
                    </div>
                    <button id="reels-next" type="button" class="absolute right-2 inline-flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-slate-700 shadow hover:bg-slate-200">›</button>
                </div>
                <div class="mt-6 flex items-center justify-between text-sm font-semibold text-slate-700">
                    <button type="button" class="rounded-full bg-slate-100 px-4 py-2 shadow hover:bg-slate-200" id="reels-prev-bottom">Previous</button>
                    <button type="button" class="rounded-full bg-indigo-500 px-4 py-2 text-white shadow hover:bg-indigo-600" id="reels-next-bottom">Next</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    (function() {
        ...
        function updateModal(idx) {
            ...
            const placeholder = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width=\"900\" height=\"1600\"><rect width=\"900\" height=\"1600\" fill=\"%230f172a\"/><text x=\"50%\" y=\"50%\" fill=\"%23ffffff\" font-size=\"48\" font-family=\"Arial\" text-anchor=\"middle\">Offer</text></svg>';
            const media = reel.images[frame] || placeholder;
            const isVideo = typeof media === 'string' && (media.endsWith('.mp4') || media.endsWith('.webm') || media.endsWith('.mov'));
            if (isVideo) {
